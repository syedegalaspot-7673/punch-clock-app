...
st.set_page_config(page_title="Punch Clock", layout="wide")
...
def connect_sheet():
    ...
    if Path("credentials.json").exists():
        ...
        return gspread.authorize(creds).open_by_key(SHEET_ID)
    st.error("Google credentials missing. Add Streamlit Secrets online, or put credentials.json beside app.py locally.")
    st.stop()
...
def get_ws(sheet, name, headers):
    ...
    if not vals:
        ...
        ws.append_row(headers)
        if old:
            ws.append_rows([(r + [""] * len(headers))[:len(headers)] for r in old])
    return ws
...
def read_ws(ws, headers):
    ...
    for r in vals[1:]:
        ...
        if r == headers or r[0].strip().lower() in ["", "id"]:
            continue
        rows.append(r)
...
def calc_logs(df):
    ...
    for i, r in df.iterrows():
        ...
        if pi is not None and po is not None:
            ...
            wh = round(max(gross - (float(bh) if bh != "" else 0), 0), 2)
        df.loc[i, "break_hours"] = bh
        df.loc[i, "work_hours"] = wh
    return df
...
if page == "Employee Clock":
    ...
    if active.empty:
        st.error("No active employees found. Add employees in Google Sheet > Employees tab.")
        st.stop()
    ...
    if emp_row is not None:
        ...
        if cur.empty:
            ...
            r = cur.iloc[-1]
            status = "On Break" if str(r["break_start"]).strip() != "" and str(r["break_end"]).strip() == "" else "Working"
    ...
    if c1.button("Punch In", use_container_width=True):
        ...
        if emp_row is None:
            ...
            if not cur.empty:
                ...
                t = now_text()
                new = pd.DataFrame([{"id": cid(emp_row["id"]), "name": emp_row["name"], "team": emp_row["team"], "work_date": str(date.today()), "punch_in": t, "break_start": "", "break_end": "", "punch_out": "", "break_hours": "", "work_hours": "", "status": "Working", "notes": notes}])
    ...
    if c2.button("Start Break", use_container_width=True):
        ...
        if emp_row is None:
            ...
            if cur.empty:
                ...
                i = cur.index[-1]
                bs, be = str(logs.loc[i, "break_start"]).strip(), str(logs.loc[i, "break_end"]).strip()
                if bs == "":
                    t = now_text()
                    ...
                    st.warning("Break already completed.")
    ...
    st.dataframe(active_rows[["id", "name", "team", "punch_in", "break_start", "break_end", "status"]], use_container_width=True)
    st.subheader("Recent Shift History")
    st.dataframe(fresh.tail(10)[["id", "name", "team", "work_date", "punch_in", "break_start", "break_end", "punch_out", "break_hours", "work_hours", "status"]], use_container_width=True)
    ...
    with t2:
        ...
        if st.button("Add Employee"):
            if not nid or not name or not epin:
                ...
                employees = pd.concat([employees, pd.DataFrame([{"id": cid(nid), "name": name, "pin": epin, "team": team, "active": is_active}])], ignore_index=True)
                save_ws(emp_ws, employees, EMP_HEADERS)
                st.success("Employee added.")
    with t3:
        raw = calc_logs(read_ws(log_ws, LOG_HEADERS))
        st.dataframe(raw, use_container_width=True)
        st.download_button("Download Attendance CSV", raw.to_csv(index=False).encode("utf-8"), "attendance.csv", "text/csv")
    ...
    summary = completed.groupby(["id", "name", "team"], as_index=False).agg(total_work_hours=("work_num", "sum"), total_break_hours=("break_num", "sum"), shifts=("id", "count"))
    ...
    st.download_button("Download Payroll CSV", summary.to_csv(index=False).encode("utf-8"), "payroll_summary.csv", "text/csv")
